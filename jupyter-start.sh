...
# Modified by Daniel Hollas to enable root execution of SLURM service,
# while still executing other services under jovyan user.
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # ISPG change: Execute before-notebook.d hooks under jovyan user!
    # Otherwise, some AiiDAlab startup scripts will fail to start.
    # Note that scripts in /usr/local/bin/start-notebook.d/ *will* execute under root,
